        ...
        <div className="space-y-6">
          ...
          <div className="bg-surface-card rounded-xl shadow-sm p-6">
            ...
            {researchPapersMut.isSuccess && (
              <div className="px-4 py-3 bg-emerald-50 rounded-lg text-sm text-emerald-700 space-y-2">
                <p>
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-emerald-700 bg-emerald-100 rounded-lg hover:bg-emerald-200 transition-colors duration-200"
                >
                  <CheckCircle className="w-4 h-4" />
                  {t('paper.researchNextStep')}
                </button>
              </div>
            )}
        ...
        <div className="space-y-6">
          {!hasOutline ? (
            ...
            <>
              ...
              {/* Outline actions — show for created/outline_draft/researched */}
              {(['created', 'outline_draft', 'researched'].includes(project.status)) && (
                <div className="bg-surface-card rounded-xl shadow-sm p-6 space-y-4">
                  <h3 className="font-heading text-lg font-semibold text-primary-950">
                    {t('paper.outlineActions')}
                  </h3>

                  {/* Regenerate outline (after research, re-generate to include new citations) */}
                  {project.status === 'researched' && (
                    <button
                      onClick={() => {
                        startOperation(project.id, project.name, 'outline')
                        regenerateOutlineMut.mutate(project.id, { onSettled: () => clearOperation() })
                      }}
                      disabled={regenerateOutlineMut.isPending}
                      className="flex items-center gap-1.5 px-4 py-2 text-sm text-white bg-primary-600 rounded-lg hover:bg-primary-700 transition-colors duration-200 disabled:opacity-50"
                    >
                      <RefreshCw className="w-4 h-4" />
                      {t('paper.regenerateOutline')}
                    </button>
                  )}
